...
export const seedDatabase = async () => {
  try {
    ...
    // 6. Create Procurements & Cross-Role Lifecycle Data
    console.log('Seeding procurements and lifecycle events...');
    
    // Statuses to distribute: REGISTERED, VERIFIED, TOKEN_GENERATED, SCHEDULED, QUALITY_CHECK, PROCURED, PAYMENT_INITIATED, PAYMENT_RECEIVED
    ...
    for (let i = 0; i < farmers.length; i++) {
      ...
      const qty = parseFloat((Math.random() * 20 + 10).toFixed(2));
      const amount = qty * crop.mspRate;
      ...
      const p = new Procurement({
        ...
        scheduledDate: new Date(Date.now() + 86400000 * (i%3))
      });
      await p.save();

      if (['TOKEN_GENERATED', 'SCHEDULED', 'QUALITY_CHECK', 'PROCURED', 'PAYMENT_RECEIVED'].includes(status)) {
        const token = new Token({
          tokenNumber: `AGRO-${2000 + i}`,
          procurementId: p._id,
          farmerId: f._id,
          centreId: centre._id,
        ...
        await token.save();
        p.tokenId = token._id;
        if(status === 'SCHEDULED' || status === 'QUALITY_CHECK') {
           p.slotStart = '10:00'; p.slotEnd = '12:00';
        }
        await p.save();
      }

      if (['PROCURED', 'PAYMENT_RECEIVED'].includes(status)) {
        const officer = officers[i % officers.length];
        await new QualityInspection({
          procurementId: p._id,
          officerId: officer._id,
          expectedQuantity: qty,
          actualWeight: qty - 0.1,
          moisture: 12.5,
          grade: 'FAQ',
          result: 'ACCEPTED'
        }).save();
        ...
        const payment = new Payment({
          ...
          quantity: qty - 0.1,
          rate: crop.mspRate,
          estimatedAmount: (qty - 0.1) * crop.mspRate,
          status: status === 'PAYMENT_RECEIVED' ? 'CREDITED' : 'PENDING'
        });
        await payment.save();
        ...
        await new Document({
          ...
          centreId: centre._id,
          type: 'RECEIPT',
          title: 'Procurement Receipt J-Form',
          status: 'GENERATED'
        }).save();
      }
    }

    // Ensure the specific demo scenario from Prompt 70 for Ramesh Kumar (9876543210)
    console.log('Setting up strict demo scenario...');
    const pDemo = new Procurement({
        farmerId: demoFarmer._id,
        centreId: centres[0]._id, // Mandi Bhawan
        cropId: crops[0]._id, // Wheat
        quantity: 18.5,
        season: 'Rabi',
        year: 2026,
        mspRate: 2425,
        status: 'SCHEDULED', // Ready for Officer Rajesh to Quality Check
        estimatedAmount: 18.5 * 2425,
    ...
    const tDemo = new Token({
        tokenNumber: 'AGRO-2048',
        procurementId: pDemo._id,
        farmerId: demoFarmer._id,
